...
//#define NOSAVEECHODATA   //不使用数组保存回波数据
...
#define REALDIAMETER 1024
#define RADIUS3 1024

#define MAXDEGREE 360

#define MAXPACKETNUM  1750  // 1250  //一圈的最大包数
//采用GL_UNSIGNED_BYTE时的格式为ABGR
 //#define ColorConvert(r, g, b)    ((quint32)(255) | ((quint32)b << 8) | ((quint32)g << 16) | ((quint32)r << 24))
#define ColorConvert(r, g, b)    ((quint32)(r) | ((quint32)b << 16) | ((quint32)g << 8) | ((quint32)(255) << 24))
...
public:
    ...
    public:
        ...
        {   m_rotation2 = rotation;   drawImageFlag = true;    }
     ...
     inline   void drawVrmEbl(QPainter *painter);
       ...
       {
           colorMode = val;
           secondEchoColor = (val == 0) ? ColorConvert(0xd4, 0xd4, 0x25) :  ColorConvert(0x30, 0xba, 0x30); //深黄色,深绿色
       }
 ...
 private:
    ...
    /***********************回波绘制相关*************************/
       ...
       private:
           ...
           //黄色回波的第二种颜色
           quint32 secondEchoColor;

           //跟踪鼠标标志
           quint8 m_mouseTracking;

           QList<quint32> echoviewCircle;   //回波背景显示
           QList<quint32> echoviewOutCircle;   //回波外部背景

           QList<quint32> guardzoneArea;   //警戒区域
...
#else
           ECHODATA m_echoDataList[MAXPACKETNUM];  //保存回波数据,最大700*2.5
#endif
           quint8 flag_break;
...
protected:
      ...
      //   virtual void paintGL();
         ...
         void  drawOuterFace(QPainter *painter);

         void updatePixels(GLuint* dst, int size);

private:

           bool pboSupported;    //pbo标志
           int pboMode;
